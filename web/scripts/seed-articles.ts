...
const run = async () => {
  ...
  for (const title of categoryNames) {
    ...
    if (existing.docs.length) {
      categoryIds[title] = existing.docs[0].id
      payload.logger.info(`category exists: ${title}`)
    } else {
      ...
      categoryIds[title] = created.id
      payload.logger.info(`category created: ${title}`)
    }
  }
  ...
  for (const [i, a] of articles.entries()) {
    ...
    if (existing.docs.length) {
      payload.logger.info(`skip (exists): ${a.slug}`)
      skipped++
      continue
    }

    const publishedAt = new Date(Date.now() - (articles.length - i) * 86400000).toISOString()
    ...
    await payload.create({
      ...
      data: {
        ...
        _status: 'published',
        publishedAt,
        categories: [categoryIds[a.category] as any],
        content: body(a.lede, a.sections) as any,
        meta: {
          title: a.title,
          description: a.meta,
        },
    ...
    payload.logger.info(`created: ${a.slug}`)
    created++
  }
