...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  CREATE TABLE "author" (
  	"id" serial PRIMARY KEY NOT NULL,
  	"name" varchar DEFAULT 'hrizonmedia',
  	"biography" varchar DEFAULT 'hrizonmedia is a secure video platform. Customers upload video; we encrypt it, store it, deliver it, and issue DRM licences so it plays on approved devices and nowhere else. These Articles come out of that work.',
  	"updated_at" timestamp(3) with time zone,
  	"created_at" timestamp(3) with time zone
  );
